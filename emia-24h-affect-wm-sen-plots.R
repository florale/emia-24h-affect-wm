...
sen_sub_models   <- c("m_valence_sen_sub", "m_energeticarousal_sen_sub", "m_calmness_sen_sub", "m_wm_sen_sub")
...
rg <- expand.grid.df(data.frame(sen_sub_models, resp), 
                     data.frame(level, level_labels),
                     data.frame(parts, part_labels))
rg <- rg[order(rg$level, rg$sen_sub_models), ]
...
# between -------------------
sen_sub_models_b <- list()

for(i in seq_along(sen_sub_models)) {
  
  model_tmp <- get(sen_sub_models[[i]])
  ...
  sen_sub_models_b[[i]] <- model_tmp
}
names(sen_sub_models_b) <- (sen_sub_models)

emia_24h_sen_plot_b <- foreach(i = 1:20,
                                .packages = "multilevelcoda") %dopar% {
                                  
                                  ggplot(sen_sub_models_b[[rg[i, "sen_sub_models"]]][To == eval(rg[i, "parts"]) & Level == eval(rg[i, "level"])], 
                                         aes(x = Delta, y = Mean)) +
...
names(emia_24h_sen_plot_b) <- foreach(i = 1:20) %dopar% {
  paste0(rg[i, "level_labels"], "Reallocations of ", rg[i, "part_labels"], " and ", rg[i, "resp"])
}
saveRDS(emia_24h_sen_plot_b, paste0(outputdir, "emia_24h_sen_plot_b", ".RDS"))

# within -------------------
sen_sub_models_w <- list()
...
for(i in seq_along(sen_sub_models)) {
  ...
  sen_sub_models_w[[i]] <- model_tmp
}
names(sen_sub_models_w) <- (sen_sub_models)

emia_24h_sen_plot_w <- foreach(i = 21:40,
                                .packages = "multilevelcoda") %dopar% {
                                  
                                  ggplot(sen_sub_models_w[[rg[i, "sen_sub_models"]]][To == eval(rg[i, "parts"]) & Level == eval(rg[i, "level"])], 
                                         aes(x = Delta, y = Mean)) +
                                    geom_hline(yintercept = 0, linewidth = 0.2, linetype = 2) +
                                    geom_vline(xintercept = 0, linewidth = 0.2, linetype = 2) +    
                                    geom_ribbon(aes(ymin = CI_low,
...
names(emia_24h_sen_plot_w) <- foreach(i = 21:40) %dopar% {
  paste0(rg[i, "level_labels"], "Reallocations of ", rg[i, "part_labels"], " and ", rg[i, "resp"])
}
saveRDS(emia_24h_sen_plot_w, paste0(outputdir, "emia_24h_sen_plot_w", ".RDS"))

## supplementary materials ----
emia_24h_sen_plot_b_supp <- foreach(i = 1:20,
                                     .packages = "multilevelcoda") %dopar% {
...
names(emia_24h_sen_plot_b_supp) <- foreach(i = 1:20) %dopar% {
  paste0(rg[i, "level_labels"], "Reallocations of ", rg[i, "part_labels"], " and ", rg[i, "resp"])
}
saveRDS(emia_24h_sen_plot_b_supp, paste0(outputdir, "emia_24h_sen_plot_b_supp", ".RDS"))

emia_24h_sen_plot_w_supp <- foreach(i = 21:40,
                                     .packages = "multilevelcoda") %dopar% {
...
names(emia_24h_sen_plot_w_supp) <- foreach(i = 21:40) %dopar% {
  paste0(rg[i, "level_labels"], "Reallocations of ", rg[i, "part_labels"], " and ", rg[i, "resp"])
}
saveRDS(emia_24h_sen_plot_w_supp, paste0(outputdir, "emia_24h_sen_plot_w_supp", ".RDS"))
